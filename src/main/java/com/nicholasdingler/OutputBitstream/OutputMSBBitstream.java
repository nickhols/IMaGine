...
public class OutputMSBBitstream extends OutputBitstream{
    public OutputMSBBitstream(OutputStreamWrapper stream){
    this.stream = stream;
}
    ...
    public void write(long bits, int nBits, boolean invertEndian) throws Exception{
        ...
        nBitsCache += nBits;
        while(nBitsCache >= 8){
            int newNBitsCache = nBitsCache - 8;
            stream.write((int)( bitCache >> newNBitsCache ) & 0xFF);
            bitCache = bitCache & ((1 << newNBitsCache) - 1);
            nBitsCache = newNBitsCache;
        }
    }
}
